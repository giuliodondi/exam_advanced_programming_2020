	...
	*/
	...
	size_t _depth( node* sub_root ) const {
		if (!sub_root) {
			...
			size_t left_depth{ _depth( sub_root->left() ) };
			size_t right_depth{ _depth( sub_root->right() ) };
	...
	bool _balanced ( node* sub_root ) const {
		...
		if (sub_root) { 
			auto depth_l = 	_depth(sub_root->left());
			auto depth_r = 	_depth(sub_root->right() );
			long int diff = depth_l - depth_r;
			flag = ((abs( diff  )<=1)  && _balanced(sub_root->left()) &&  _balanced(sub_root->right()) );
		}
		return flag;
	}
...
/*
template <typename K, typename T, typename CMP >
node*  bst<K,T, CMP>::walk(node* root) { }
*/
